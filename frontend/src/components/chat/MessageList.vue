...
<template>
  ...
  >
    ...
    <div v-if="messagesStore.loadingMore" class="flex justify-center py-3">
      <svg class="animate-spin w-5 h-5 text-atlas-500" fill="none" viewBox="0 0 24 24">
        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
      </svg>
    </div>
    <p v-else-if="!hasMore && messages.length > 0" class="text-center text-xs text-dark-600 py-2">
    ...
    <div v-else class="flex flex-col items-center justify-center h-full text-center">
      <div class="w-16 h-16 mb-4 rounded-2xl bg-dark-800 flex items-center justify-center">
        <svg class="w-8 h-8 text-atlas-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
        </svg>
      </div>
      <h3 class="text-lg font-semibold text-dark-200 mb-1">Начните общение!</h3>
