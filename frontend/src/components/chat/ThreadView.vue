  ...
  >
    ...
    <div class="px-4 py-3 border-b border-dark-800 flex items-center justify-between">
      <div class="flex items-center gap-2">
        ...
        <h3 class="font-semibold text-dark-100">Тред</h3>
      </div>
      <button
        @click="closeThread"
    ...
    <!-- Parent message -->
    <div v-if="parentMessage" class="px-4 py-3 border-b border-dark-800 bg-dark-850">
    ...
    <div class="flex-1 overflow-y-auto px-4 py-3 space-y-1">
      ...
      <!-- Empty state -->
      <div v-if="threadStore.currentThread?.length === 0 && !threadStore.loading" class="text-center py-8">
        ...
        <p class="text-dark-500 text-sm">Пока нет ответов</p>
        <p class="text-dark-600 text-xs mt-1">Начните обсуждение</p>
      </div>

      <!-- Loading -->
      <div v-if="threadStore.loading" class="flex justify-center py-8">
    ...
    <div v-if="parentMessage" class="border-t border-dark-800">
      <MessageInput
